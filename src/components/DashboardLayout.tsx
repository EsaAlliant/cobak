...
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  return <div className="admin-shell"><button className="mobile-menu" type="button" onClick={() => setOpen(true)} aria-label="Buka menu" aria-expanded={open}><i className="fa-solid fa-bars" /></button><div className={`admin-drawer ${open ? "open" : ""}`}><Sidebar role={session.role} name={session.name} avatarUrl={session.avatarUrl} onLogout={logout} onNavigate={() => setOpen(false)} /></div>{open ? <button className="drawer-backdrop" onClick={() => setOpen(false)} aria-label="Tutup menu" /> : null}<main className="admin-main"><header className="admin-topbar"><div><span>CMS {settings.namaWebsite}</span><h1>{settings.tagline}</h1></div><button type="button" onClick={logout}><i className="fa-solid fa-arrow-right-from-bracket" /> Keluar</button></header>{children}</main></div>;
}
