...
export const websiteSettingsSchema = z.object({
  ...
  stats_mode: z.enum(["auto", "manual"]), stat_population: z.coerce.number().int().min(0).max(10000000), stat_households: z.coerce.number().int().min(0).max(1000000), stat_umkm: z.coerce.number().int().min(0).max(1000000), stat_gallery: z.coerce.number().int().min(0).max(1000000), stat_agenda: z.coerce.number().int().min(0).max(1000000),
});
